...
class PublishingEngine extends DataExtension
{
    /**
     * These constants define context actions.
     */
    const CONTEXT_ACTION = 'action';
    const CONTEXT_ACTION_PUBLISH = 'publish';
    const CONTEXT_ACTION_UNPUBLISH = 'unpublish';
    ...
    public function collectChanges($context) {
        ...
        if (is_callable([$this->owner, 'objectsToUpdate'])) {

            $toUpdate = $this->owner->objectsToUpdate($context);

            if ($toUpdate) foreach ($toUpdate as $object) {
                if (!is_callable([$this->owner, 'urlsToCache'])) continue;

                $urls = $object->urlsToCache($context);
                if(!empty($urls)) {
    ...
    public function convertUrlsToPathMap($urls) {
        ...
        foreach($urls as $url) {
            ...
            if (!$obj || !is_callable([$obj, 'Subsite'])) {
                // Normal processing for files directly in the cache folder.
                $pathMap = array_merge($pathMap, $this->owner->urlsToPaths([$url]));
                ...
                if (!$subsite || !$subsite->ID) {
                    ...
                    $pathMap = array_merge($pathMap, $this->owner->urlsToPaths([$staticBaseUrl . '/' . $cleanUrl]));
                } else {
